...
- RoPE: Allows the model to generalize to sequence lengths slightly longer than training and handles relative distances between the three sentences better than absolute embeddings.
- Pre-Norm + RMSNorm: standard in Llama/Mistral architectures for training stability.
- No Biases in Linear layers. Saves parameters without hurting performance.
